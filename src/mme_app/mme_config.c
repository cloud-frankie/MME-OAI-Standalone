...
static void mme_config_init (mme_config_t * config_pP)
{
  ...
  // todo: sgw address?
  config_pP->ipv4.pgw_s5.s_addr = INADDR_ANY;
...
static int mme_config_parse_file (mme_config_t * config_pP)
{
  ...
  char                                   *pgw_ip_address_for_s5 = NULL;
...
//      }
    ...
    // pgw static selection
    setting = config_setting_get_member (setting_mme, MME_CONFIG_STRING_PGW_CONFIG);

    if (setting != NULL) {
        if ((config_setting_lookup_string (setting, MME_CONFIG_STRING_PGW_IPV4_ADDRESS_FOR_S5, (const char **)&pgw_ip_address_for_s5)
            )
        ) {
            cidr = bfromcstr (pgw_ip_address_for_s5);
            struct bstrList *list = bsplit (cidr, '/');
            AssertFatal(2 == list->qty, "Bad CIDR address %s", bdata(cidr));
            address = list->entry[0];
            mask    = list->entry[1];
            IPV4_STR_ADDR_TO_INADDR (bdata(address), config_pP->ipv4.pgw_s5, "BAD IP ADDRESS FORMAT FOR PGW S5 !\n");
            config_pP->ipv4.netmask_s5 = atoi ((const char*)mask->data);
            bstrListDestroy(list);
            bdestroy_wrapper(&cidr);
            in_addr_var.s_addr = config_pP->ipv4.pgw_s5.s_addr;
            OAILOG_INFO (LOG_MME_APP, "Parsing configuration file found P-GW S5: %s\n", inet_ntoa (in_addr_var));
        }
    }    
  }
...
static void mme_config_display (mme_config_t * config_pP)
{
  ...
  // Added by Entel
  OAILOG_INFO (LOG_CONFIG, "- S5:\n");
  OAILOG_INFO (LOG_CONFIG, "    s5-PGW ip ........: %s\n", inet_ntoa (*((struct in_addr *)&config_pP->ipv4.pgw_s5)));
