...
//  memset (session_request_p, 0, sizeof (itti_s11_create_session_request_t));
  ...
  //entel change for PGW TEID

  //OAI_GCC_DIAG_OFF(pointer-to-int-cast);
  //session_request_p->pgw_address_for_cp.teid = (teid_t) 0x000000; /**< Which does not matter. */
  //OAI_GCC_DIAG_ON(pointer-to-int-cast);
  //session_request_p->pgw_address_for_cp.interface_type = S5_S8_PGW_GTP_C;
  //mme_config_read_lock (&mme_config);
  //session_request_p->pgw_address_for_cp.ipv4_address = mme_config.ipv4.pgw_s5;
  //mme_config_unlock (&mme_config);
  //session_request_p->pgw_address_for_cp.ipv4 = 1;

  //ue_context->mme_teid_s11 = session_request_p->sender_fteid_for_cp.teid;
  DevAssert(!pdn_context->s_gw_teid_s11_s4);

  //HERE STARTS ENTEL MODIFICATION
  ...
  //const_bstring apnoi = bfromcstr (".MNC017.MCC716.GPRS");
  //bstring fqdn = bstrcpy(pdn_context->apn_subscribed);
  //bconcat(fqdn, apnoi);
  //memcpy (session_request_p->apn, fqdn->data, blength(fqdn)) ;
  //end of Entel modification
